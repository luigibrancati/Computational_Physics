...
double dE(int i, int j){
	...
	//up and down, open boundary
	if(i==0) coeff+=lattice[1][j];
	else if(i==L-1) coeff+=lattice[L-2][j];
	else coeff+=lattice[i-1][j]+lattice[i+1][j];

	//left and right, open boundary
	if(j==0) coeff+=lattice[i][1];
	else if(j==L-1) coeff+=lattice[i][L-2];
