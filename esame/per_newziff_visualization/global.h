/*
	...
	Da usare per la visualizzazione, immagini ecc.
*/
#ifndef PERCOLATION
#define PERCOLATION
...
class latt{
	...
	long int secondmoment; 	//secondo momento=somma{s^2*n_s}, s dimensione e n_s numero di cluster con dimensione s, serve per la dimensione media dei cluster
...
int latt::meansize(){
	...
	if(occupiedcorrect>0){
		if (secondmoment<0) cout << "sec: " << secondmoment << endl;
